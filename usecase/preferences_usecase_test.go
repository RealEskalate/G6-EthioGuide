...
func (s *PreferencesUsecaseSuite) TestCreateUserPreferences_Error() {
	// Arrange
	ctx := context.Background()
	expectedError := errors.New("database create failed")
	s.mockRepo.On("Create", mock.Anything, mock.AnythingOfType("*domain.Preferences")).Return(expectedError).Once()
...
func (s *PreferencesUsecaseSuite) TestGetUserPreferences_Error() {
	// Arrange
	ctx := context.Background()
	userID := "user404"
	expectedError := errors.New("user preferences not found")
	s.mockRepo.On("GetByUserID", mock.Anything, userID).Return(nil, expectedError).Once()

	// Act
	got, err := s.usecase.GetUserPreferences(ctx, userID)
...
func (s *PreferencesUsecaseSuite) TestUpdateUserPreferences_Error() {
	// Arrange
	ctx := context.Background()
	pref := &domain.Preferences{
		UserID: "user123",
	...
	s.mockRepo.On("UpdateByUserID", mock.Anything, pref.UserID, pref).Return(expectedError).Once()
